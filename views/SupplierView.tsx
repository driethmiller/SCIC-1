...
import { SupplierMock } from '../types';
import { fetchSuppliers } from '../services/mockDataService';
...
const SuppliersView: React.FC = () => {
  ...
  const [data, setData] = useState<SupplierMock[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  // fix: Use SupplierMock for state.
  const [editingSupplier, setEditingSupplier] = useState<SupplierMock | null>(null);
  ...
  const handleOpenModal = (supplier: SupplierMock | null = null) => {
  ...
  // fix: Use SupplierMock for parameter.
  const handleSave = (formData: Omit<SupplierMock, 'cageCode'> & { cageCode?: string }) => {
    if (editingSupplier) {
      // Update
      setData(data.map(s => s.cageCode === editingSupplier.cageCode ? { ...s, ...formData } as SupplierMock : s));
    } else {
      // Create
      const newSupplier: SupplierMock = {
        ...formData,
        cageCode: formData.cageCode || `NEW-${Date.now()}` // Simple unique ID generation
      };
  ...
  const handleDelete = (cageCode: string) => {
    if (window.confirm('Are you sure you want to delete this supplier?')) {
      setData(data.filter(s => s.cageCode !== cageCode));
    }
  };


  // fix: Use SupplierMock for column definitions.
  const columns: Column<SupplierMock>[] = [
    { header: 'CAGE Code', accessor: 'cageCode' },
    { header: 'Supplier Name', accessor: 'companyName' },
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && !error && data && (
        data.length > 0 ? (
          <DataTable data={data} columns={columns} keyAccessor="cageCode" />
        ) : (
...
interface SupplierFormModalProps {
    ...
    onSave: (data: Omit<SupplierMock, 'cageCode'> & { cageCode?: string }) => void;
    supplier: SupplierMock | null;
}
...
const SupplierFormModal: React.FC<SupplierFormModalProps> = ({ isOpen, onClose, onSave, supplier }) => {
    ...
    const [formData, setFormData] = useState<Omit<SupplierMock, 'cageCode' | 'lat' | 'lon'>>({
        companyName: '',
        address: '',
        city: '',
        province: '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        onSave(supplier ? { ...formData, cageCode: supplier.cageCode } : formData);
    };
